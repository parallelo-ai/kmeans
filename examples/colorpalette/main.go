...
func (c Color) Coordinates() kmeans.Coordinates {
	l, a, b := c.Lab()
	return kmeans.Coordinates{l, a, b}
}

func (c Color) Distance(pos kmeans.Coordinates) float64 {
...
func main() {
	...
	km, _ := kmeans.NewKmeansWithOptions(0.01, kmeans.DefaultIterationThreshold)

	// Partition the color space into 16 kmeans (palette colors)
	kmeans, _ := km.Partition(d, 16, 0)

	for i, c := range kmeans {
